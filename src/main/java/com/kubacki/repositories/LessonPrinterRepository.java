...
public interface LessonPrinterRepository extends JpaRepository<Lesson, Integer>{
    @Query(value = "select new com.kubacki.domain.LessonList(l.id, s.name, lt.nazwa,l.start_date,l.end_date,l.start_hour,l.end_hour,s.colour) from Lesson l, Subject s, LessonType lt where l.subject.id = s.id and l.lessonType.id = lt.id")
